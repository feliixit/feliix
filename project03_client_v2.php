...
<head>
    ...
    <style scoped>
        ...
        div.block.left.first {
            margin-right: 28px;
            width: calc(45% - 14px);
        }

        div.block.left.second {
            margin-right: 0;
            width: calc(55% - 14px);
        }
    ...
    <div class="mainContent" id='app'>
        ...
        <div class="block left first">
            ...
            <div class="tablebox lv3a">
                ...
                <ul>
                    <li>
                        <div v-for='(receive_record, index) in stage_client_venue' style="white-space: pre-wrap;">{{ receive_record.message }} ({{
...
?>></a>
                ...
                <ul>
                    <li>
                        <div v-for='(receive_record, index) in stage_client_competitor' style="white-space: pre-wrap;">{{ receive_record.message }} ({{
                            receive_record.username }} at {{ receive_record.created_at }})
                        </div>
                    </li>
                    <li>
...
?>></a>
                ...
                <ul>
                    <li>
                        <div v-for='(receive_record, index) in stage_client_infomation' style="white-space: pre-wrap;">{{ receive_record.message }} ({{
                            receive_record.username }} at {{ receive_record.created_at }})
                ...
                <div class="teskbox dialogclear" style="margin-top:-2px !important">
                    <div class="tablebox m01">
                        <ul>
                            <li><b>Creator</b></li>
                            <li><a class="man"
                                   :style="'background-image: url(images/man/' +  receive_record.creator_pic  + ');'"
                                   :title="receive_record.creator"></a></li>
                        </ul>
                        <ul>
                            <li><b>Assignee</b></li>
                            <li>
                                <i v-for="item in receive_record.assignee">
                                    <a class="man" :style="'background-image: url(images/man/' + item.pic_url + ');'"
                                       :title="item.username"></a>
                                </i>
                        ...
                        <ul>
                            ...
                            <li>
                                <i v-for="item in receive_record.items">
                                    <a v-if="item.gcp_name.split('.').pop().toLowerCase() === 'jpg' || item.gcp_name.split('.').pop().toLowerCase() === 'png'"
                                       class="attch_pic" :href="baseURL + item.gcp_name" target="_blank"><img
                                            :src="baseURL + item.gcp_name"></a>
                                    <a v-if="item.gcp_name.split('.').pop().toLowerCase() !== 'jpg' && item.gcp_name.split('.').pop().toLowerCase() !== 'png'"
                                       class="attch" :href="baseURL + item.gcp_name"
                    ...
                    <div class="tableframe">
                        <div class="tablebox m02">
                            ...
                            <ul v-for="item in receive_record.message"
                                :class="{ deleted : item.message_status == -1, dialogclear : item.message_status == -1 }">
                                <li class="dialogclear">
                                    <a class="man"
                                       :style="'background-image: url(images/man/' + item.messager_pic + ');'"
                                       :title="item.messager"></a>
                                    <i class="info">
                                        ...
                                        <div class="msgbox dialogclear">
                                            ...
                                            <i v-for="file in item.items">
                                                <a v-if="file.gcp_name.split('.').pop().toLowerCase() === 'jpg' || file.gcp_name.split('.').pop().toLowerCase() === 'png'"
                                                   ...
                                                   target="_blank"><img :src="baseURL + file.gcp_name"></a>
                                                <a v-if="file.gcp_name.split('.').pop().toLowerCase() !== 'jpg' && file.gcp_name.split('.').pop().toLowerCase() !== 'png'"
                                                   class="attch" :href="baseURL + file.gcp_name" target="_blank">{{file.filename}}</a>
                                            </i>
                                            <div class="already_read">
                                                <template v-for="(got, index) in item.got_it">{{ got.username }}<span
                                                        v-if="index + 1 < item.got_it.length">, </span></template>
                                            </div>

                                        </div>
                                        <div class="btnbox">
                                            <a class="btn small green"
                                               @click="got_it_message(item.message_id, item.ref_id)"
                                               v-if="item.i_got_it == false">Got it</a>
                                            <a class="btn small green reply r1" v-if="item.ref_id == 0"
                                               :id="'task_reply_btn_' + item.message_id + '_' + item.ref_id"
                                               @click="openTaskMsgDlg(item.message_id + '_' + item.ref_id)">Reply</a>
                                            <!-- dialog -->
                                            <div class="dialog reply r1"
                                                 :id="'task_reply_dlg_' + item.message_id + '_' + item.ref_id"
                                                 :style="item.i_got_it == false ? 'top: 17px;' : ''">
                                                <div class="formbox">
                                                    <dl>
                                                        <dd><textarea name=""
                                                            ...
                                                            <div class="pub-con" ref="bg">
                                                                <div class="input-zone">
                                                                    ...
                                                                    <input class="input" type="file"
                                                                           :ref="'file_msg_' + item.message_id + '_' + item.ref_id"
                                                                           placeholder="choose file"
                                                                           @change="changeMsgFile(item.message_id + '_' + item.ref_id)"
                                                                           multiple/>
                                                                </div>
                                                            </div>
                                                        </dd>
                                                        <dd>
                                                            <div class="filebox">
                                                                <a class="attch"
                                                                   v-for="(it,index) in msgItems(item.message_id + '_' + item.ref_id)"
                                                                   :key="index"
                                                                   @click="deleteMsgFile(item.message_id + '_' + item.ref_id, index)">{{it.name}}</a>
                                                            </div>
                                                        </dd>
                                                        <dd>
                                                            <div class="btnbox">
                                                                <a class="btn small orange"
                                                                   @click="msg_clear(item.message_id + '_' + item.ref_id)">Cancel</a>
                                                                <a class="btn small green"
                                                                   @click="msg_create(item.message_id + '_' + item.ref_id, item.message_id)">Save</a>
                                                            </div>
                                                        </dd>
                                                    </dl>
                                                </div>
                                            ...
                                            <!-- dialog end -->
                                            <a class="btn small yellow"
                                               @click="msg_delete(item.message_id, item.ref_id, item.messager_id, '<?php echo $user_id; ?>')">Delete</a>
                                        </div>
                        ...
                        <ul>
                            <li>
                                ...
                                <div class="filebox">
                                    <a class="attch" v-for="(item,index) in taskItems(receive_record.task_id)"
                                       :key="index"
                                       @click="deleteTaskFile(receive_record.task_id, index)">{{item.name}}</a>
                            ...
                            <li>
                                <div class="pub-con" ref="bg">
                                    <div class="input-zone">
                                        <span class="upload-des">choose file</span>
                                        <input class="input" type="file" :ref="'file_task_' + receive_record.task_id"
                                               placeholder="choose file"
    ...
    <form id="addmeeting-form" style="display: none;">
        <fieldset>
            ...
            <div class="meetingform-item" id="upload_input">
                <label>File:</label>
                <input type="file" ref="file" id="fileload" name="file[]" onChange="onChangeFileUpload(event)"
                       multiple>
            </div>
...
<script type="text/javascript" src="js/project03_client_v2.js" defer></script>
