...
$merged_results = array();
            ...
            if($c != "")
            {
                $sql = $sql . " and (p.category = '" . $c . "' ) ";
                $query_cnt = $query_cnt . " and (p.category = '" . $c . "' ) ";
            }

            if($sc != "")
            {
                $sql = $sql . " and (p.sub_category = '" . $sc . "') ";
...
// $phpWord->setDefaultParagraphStyle(['lineHeight' => 0.5]);
...
$table->addRow();
$table->addCell(4500, ['borderSize' => 6])->addText("Information", array('bold' => true), array('align' => 'center', 'valign' => 'center'));
$table->addCell(4500, ['borderSize' => 6])->addText("Specification", array('bold' => true), array('align' => 'center', 'valign' => 'center'));
$table->addCell(4500, ['borderSize' => 6])->addText("Price", array('bold' => true), array('align' => 'center', 'valign' => 'center'));
...
foreach ($merged_results as &$value) {
    ...
    // information
    $cell = $table->addCell(4500, ['borderSize' => 6]);
    $cell->addText('ID: ' . htmlspecialchars($value['id']), ['bold' => false]);
    $cell->addText('Code: ' . htmlspecialchars($value['code']), ['bold' => false]);
    $cell->addText('Category: ' . htmlspecialchars($value['category'] . ' >> ' . $value['sub_category_name']), ['bold' => false]);
    $tags = "";
    foreach ($value['tags'] as $v) {
        $tags .= $v . '; ';
    }
    $tags = substr($tags, 0, -2);
    $cell->addText('Tags: ' . htmlspecialchars($tags), ['bold' => false]);
    $cell->addText('Brand: ' . htmlspecialchars($value['brand']), ['bold' => false]);
    $cell->addText('Created: ' . htmlspecialchars($value['created_at']) . ' (' . $value['created_name'] . ')', ['bold' => false]);
    if($value['updated_at'] != "")
        $cell->addText('Updated: ' . htmlspecialchars($value['updated_at']) . ' (' . $value['updated_name'] . ')', ['bold' => false]);
    else
        $cell->addText('Updated: ' . htmlspecialchars($value['updated_at']), ['bold' => false]);

    // Specification
    $cell = $table->addCell(4500, ['borderSize' => 6]);
    foreach ($value['attribute_list'] as $v) {
        $attribue = $v['category'] . ': ' . implode('; ', $v['value']);
        $cell->addText(htmlspecialchars($attribue), ['bold' => false]);
    }

    // Price
    $cell = $table->addCell(4500, ['borderSize' => 6]);
    $cell->addText('SRP: ' . htmlspecialchars($value['price']), ['bold' => false]);
    if($value['str_price_change'] != "")
        $cell->addText(htmlspecialchars($value['str_price_change']), ['bold' => false]);

    $cell->addText(' ', ['bold' => false]);

    $cell->addText('QP: ' . htmlspecialchars($value['quoted_price']), ['bold' => false]);
    if($value['str_quoted_price_change'] != "")
        $cell->addText(htmlspecialchars($value['str_quoted_price_change']), ['bold' => false]);
}
