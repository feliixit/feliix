...
var app = new Vue({
	...
	methods:{
		getReceiveRecords: function() {
            ...
            axios.get('../api/admin/project_stage', { params, headers: {"Authorization" : `Bearer ${token}`} })
            .then(
                (res) => {
        ...
        editReceiveRecord: function(event) {
            ...
            if (this.record.stage == "") 
            {
              this.error_username = '狀態需輸入 (stage required)';
              $(window).scrollTop(0);
              return false;
            } 
