...
var app = new Vue({
	...
	methods:{
		getReceiveRecords: function() {
            ...
            axios.get('../api/admin/project_status', { params, headers: {"Authorization" : `Bearer ${token}`} })
            .then(
                (res) => {
        ...
        editReceiveRecord: function(event) {
            ...
            if (this.record.project_status == "") 
            {
              this.error_username = '狀態需輸入 (project_status required)';
              $(window).scrollTop(0);
              return false;
            } 

            formData.append('project_status', this.record.project_status)
        ...
        createReceiveRecord: function() {
          let _this = this;
            ...
            {
              Swal.fire({
                text: '狀態需輸入 (project_status required)',
                icon: 'error',
