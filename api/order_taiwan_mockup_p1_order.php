...
try{
    ...
    mockup_notification($user_name, 'access1,access3,access4,access5', 'access2', $project_name, $serial_name, $od_name, 'Order - Mockup', $comment, $action, $items_array, $od_id);
...
function UpdateProduct($od_id, $item, $od_name,  $db)
{
    ...
    {
        if($v4 != '')
            $query = "update product set last_order = :od_id, last_order_name = :od_name, last_order_type = 'mockup', last_order_at = now() where product_id = :pid and 1st_variation like '%" . $v1 . "' and 2rd_variation like '%" . $v2 . "' and 3th_variation like '%" . $v3 . "' and 4th_variation like '%" . $v4 . "' ";
        else
            $query = "update product set last_order = :od_id, last_order_name = :od_name, last_order_type = 'mockup', last_order_at = now() where product_id = :pid and 1st_variation like '%" . $v1 . "' and 2rd_variation like '%" . $v2 . "' and 3th_variation like '%" . $v3 . "' ";
        // prepare the query
        $stmt = $db->prepare($query);
    ...
    if($v1 == '' && $v2 == '' && $v3 == '' && count($ps_var) == 0)
    {
        // update product_category
        $pid = $item['pid'];
        $query = "update product_category set last_order = :od_id, last_order_name = :od_name, last_order_type = 'mockup', last_order_at = now() where id = :pid ";
        // prepare the query
        $stmt = $db->prepare($query);
...
function UpdateProductQty($od_id, $item, $db)
{
    ...
    {
        $new_incoming_qty += $qty + $backup_qty;
        $new_incoming_element[] = array('od_id' => $od_id, 'qty' => $qty, 'backup_qty' => $backup_qty, 'v1' => $v1, 'v2' => $v2, 'v3' => $v3, 'v4' => $v4, 'ps_var' => $ps_var, 'order_date' => date("Y-m-d H:i:s"), 'order_type' => 'mockup');
    }
