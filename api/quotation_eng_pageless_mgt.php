...
function GetRecentPost($quotation_id, $db){
    $query = "SELECT username, p.updated_at FROM quotation_eng p LEFT JOIN user u ON p.updated_id = u.id  WHERE p.id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at  FROM quotation_eng_general_requirement p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM quotation_eng_consumable p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM quotation_eng_payment_term p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.updated_at FROM quotation_eng_signature p LEFT JOIN user u ON p.updated_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM quotation_eng_term p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM quotation_eng_total p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM quotation_eng_consumable p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM quotation_eng_installation p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    ";
