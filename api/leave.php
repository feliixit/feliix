...
while ($row = $stmt->fetch(PDO::FETCH_ASSOC)){
    if (isset($row)){
        ...
        $subquery = "SELECT u.id, u.username, l.start_date, l.start_time, l.end_date, l.end_time, l.leave_type, CASE when leave_type = 'D' then 'D' WHEN reject_id + re_reject_id > 0 THEN 'R' WHEN approval_id * re_approval_id > 0 THEN 'A'  WHEN approval_id * re_approval_id = 0 THEN 'P' END approval FROM user u LEFT JOIN apply_for_leave l ON u.id = l.uid WHERE  start_date <= '" . $mdate . "' and end_date >= '" . $mdate . "' AND l.status <> -1 and l.uid = " . $row['id'];
