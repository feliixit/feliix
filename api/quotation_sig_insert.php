...
else
{
    try {
        ...
        // quotation_page
        $query = "UPDATE quotation set pixa_s = :pixa_s, show_s = :show_s, pageless = :pageless
                WHERE
                `id` = :quotation_id";
        ...
        // bind the values
        $stmt->bindParam(':pixa_s', $pixa);
        $stmt->bindParam(':show_s', $show);
        ...
        // quotation_page
        $query = "DELETE FROM quotation_signature
                WHERE
        ...
        {
            $query = "INSERT INTO quotation_signature
            SET
                `quotation_id` = :quotation_id,
                `page` = :page,
                `type` = 'C',
            ...
            $stmt->bindParam(':email', $detail_array['item_client'][$i]['email']);
        ...
        for($i=0 ; $i < count($detail_array['item_company']) ; $i++)
        {
            $query = "INSERT INTO quotation_signature
            SET
                `quotation_id` = :quotation_id,
            ...
            if($detail_array['item_company'][$i]['photo'] !== '')
            {
                $query .= ", `photo` = :photo";
            }
            ...
            $stmt->bindParam(':email', $detail_array['item_company'][$i]['email']);
          
            $stmt->bindParam(':create_id', $user_id);

            if($detail_array['item_company'][$i]['photo'] !== '')
            {
                $stmt->bindParam(':photo', $detail_array['item_company'][$i]['photo']);
            }
            ...
            $_id = $detail_array['item_company'][$i]['id'];

                $batch_id = $last_id;
                $batch_type = "sig_image";
...
function UpdateImageNameVariation($upload_name, $batch_id, $db){
    ...
    $query = "update quotation_signature
