...
switch ($method) {
    case "GET":
        ...
        LEFT JOIN gcp_storage_file f ON f.batch_id = pm.id AND f.batch_type = 'other_task_sv'
    ...
    case 'POST':
        ...
        $query = "INSERT INTO project_other_task_sv
        SET
            `stage_id` = :stage_id,
            `title` = :title,
            `priority` = :priority,
...
function SendNotifyMail($last_id)
{
    ...
    task_notify_admin_sv("create", $project_name, $task_name, $stages, $create_id, $assignee, $collaborator, $due_date, $detail, $last_id, 0, 0, $created_at);
...
function GetMessage($task_id, $db, $uid)
{
    $sql = "select pmsgrp.id message_id, pmsgrp.message message, pmsgrp.`status` message_status, r.id uid, r.username messager, r.pic_url messager_pic, pmsgrp.created_at message_date, COALESCE(p.username, '') updator, COALESCE(pmsgrp.updated_at, '') update_date,
            ...
            from project_other_task_message_sv pmsgrp 
            LEFT JOIN user r ON r.id = pmsgrp.create_id 
            LEFT JOIN user p ON p.id = pmsgrp.updated_id 
            LEFT JOIN gcp_storage_file h ON h.batch_id = pmsgrp.id AND h.batch_type = 'other_task_msg_sv' 
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        // got it
        $got_it = GetGotIt($row['message_id'], 'sv', $db);
        foreach ($got_it as $g) {
            if ($g['uid'] == $uid) {
                $i_got_it = true;
                break;
...
function GetReply($msg_id, $db, $id, $name, $msg, $uid)
{
    $sql = "select pmsgrp.id replay_id, pmsgrp.message reply, pmsgrp.`status` reply_status, r.id uid, r.username replyer, r.pic_url replyer_pic, pmsgrp.created_at reply_date, COALESCE(p.username, '') updator, COALESCE(pmsgrp.updated_at, '') update_date,
            ...
            from project_other_task_message_reply_sv pmsgrp 
            LEFT JOIN user r ON r.id = pmsgrp.create_id 
            LEFT JOIN user p ON p.id = pmsgrp.updated_id 
            LEFT JOIN gcp_storage_file h ON h.batch_id = pmsgrp.id AND h.batch_type = 'other_task_msg_rep_sv' 
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        // got it
        $got_it = GetGotItReply($row['replay_id'], 'sv', $db);
