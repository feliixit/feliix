...
switch ($method) {
    ...
    case 'POST':
        ...
        // delete voting_template_detail
        $query = "delete from voting_template_detail where template_id = :template_id";
        try{
            $stmt = $db->prepare($query);
        ...
        try {
            for($i=0 ; $i < count($block_array) ; $i++)
            {
                ...
                $pic = isset($block_array[$i]['pic']) ? $block_array[$i]['pic'] : '';
