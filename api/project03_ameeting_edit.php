...
try{
    ...
    // delete attached_layout
    if($attached_layout == "false" || $attached_layout == "0" || $attached_layout == "f")
    {
        for($j=0; $j < count($attached_layout_file_array); $j++)
        {
            $bid = $attached_layout_file_array[$j]['id'];
            if($bid != 0)
            {
                $query = "update  gcp_storage_file
                        ...
                        WHERE
                            `id` = :_id
                            and `batch_type` = 'attached_layout' ";

                // prepare the query
                $stmt = $db->prepare($query);

                // bind the values
                $stmt->bindParam(':deleted_id', $uid);
                $stmt->bindParam(':_id', $bid);
    ...
    if($attached_layout == "true" || $attached_layout == "1" || $attached_layout == "t")
    {
        for($j=0; $j < count($attached_layout_file_array); $j++)
        {
            $check = $attached_layout_file_array[$j]['checked'];
            $bid = $attached_layout_file_array[$j]['id'];
            if($bid != 0 && $check == false)
            {
                $query = "update  gcp_storage_file
                            set `status` = -1,
                                `deleted_id` = :deleted_id,
    ...
    // delete scope_attached_layout
    if($scope_attached_layout == "false" || $scope_attached_layout == "0" || $scope_attached_layout == "f")
    {
        for($j=0; $j < count($scope_attached_layout_file_array); $j++)
        {
            $bid = $scope_attached_layout_file_array[$j]['id'];
            if($bid != 0)
            {
                $query = "update  gcp_storage_file
                            set `status` = -1,
                        ...
                        WHERE
                            `id` = :_id
                            and `batch_type` = 'scope_attached_layout' ";
    ...
    if($scope_attached_layout == "true" || $scope_attached_layout == "1" || $scope_attached_layout == "t")
    {
        for($j=0; $j < count($scope_attached_layout_file_array); $j++)
        {
            $check = $scope_attached_layout_file_array[$j]['checked'];
            $bid = $scope_attached_layout_file_array[$j]['id'];
            if($bid != 0 && $check == false)
            {
                $query = "update  gcp_storage_file
    ...
    // delete other_attached_layout
    for($j=0; $j < count($other_attached_layout_file_array); $j++)
    {
        $check = $other_attached_layout_file_array[$j]['checked'];
        $bid = $other_attached_layout_file_array[$j]['id'];
        if($bid != 0 && $check == false)
        {
            $query = "update  gcp_storage_file
                        set `status` = -1,
                    ...
                    WHERE
                        `id` = :_id
                        and `batch_type` = 'other_attached_layout' ";
