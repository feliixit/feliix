...
try{
    ...
    if($item_id != 0)
    {
        $query = "update project_other_task_message_reply_r
        SET
            status = -1,
    ...
    {
        $query = "update project_other_task_message_r
        SET
            status = -1,
            updated_id = :updated_id,
            updated_at = now()
...
function SendNotifyMail($last_id, $uid)
{
    ...
    message_notify_r("del", $project_name, $task_name, $stages, $create_id, $assignee, $collaborator, "", $detail, $stage_id,  $msg, $username, $created_at, $_id);
...
function GetTaskDetail($id, $db)
{
    $sql = "SELECT ps.id stage_id, project_name, title task_name, 
                ...
                pmsg.created_at,
                pmsg.create_id _id
                FROM project_other_task_message_r pmsg
