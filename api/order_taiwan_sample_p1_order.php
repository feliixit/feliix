...
try{
    ...
    order_type_notification($user_name, 'access1,access3,access4,access5', 'access2', $project_name, $serial_name, $od_name, 'Order - Samples', $comment, $action, $items_array, $od_id, "sample");
...
function UpdateProduct($od_id, $item, $od_name,  $db)
{
    ...
    if($v1 == '' && $v2 == '' && $v3 == '' && $v4 == '' && count($ps_var) == 0)
    {
        // update product_category
        $pid = $item['pid'];
        $query = "update product_category set last_order = :od_id, last_order_name = :od_name, last_order_type = 'samples', last_order_at = now() where id = :pid ";
        // prepare the query
        $stmt = $db->prepare($query);
