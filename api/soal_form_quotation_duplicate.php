...
function InsertQuotation($id, $user_id, $merged_results, $db)
{
    ...
    $third_line = $merged_results[0]['third_line'];
    ...
    $statement_date = $merged_results[0]['statement_date'];
    $mode = $merged_results[0]['mode'];
    $mode_content = $merged_results[0]['mode_content'];
    $caption = $merged_results[0]['caption'];
    $account_summary = $merged_results[0]['account_summary'];
    $caption_first_line = $merged_results[0]['caption_first_line'];
    $content_first_line = $merged_results[0]['content_first_line'];
    $caption_second_line = $merged_results[0]['caption_second_line'];
    $content_second_line = $merged_results[0]['content_second_line'];
    ...
    $query = "INSERT INTO soa_quotation
        ...
        $query = "INSERT INTO soa_quotation_total
            ...
            $query = "INSERT INTO soa_quotation_term
                    ...
                    from soa_quotation_term where quotation_id = :quotation_id";
            // prepare the query
            $stmt = $db->prepare($query);
            ...
            $query = "INSERT INTO soa_quotation_payment_term
                    ...
                    from soa_quotation_payment_term where quotation_id = :quotation_id";
            // prepare the query
            $stmt = $db->prepare($query);
            ...
            $query = "INSERT INTO soa_quotation_signature
                    ...
                    from soa_quotation_signature where quotation_id = :quotation_id";
            // prepare the query
            $stmt = $db->prepare($query);
...
function GetQuotation($id, $db) {
    ...
    $query = "SELECT id, 
                    ...
                    signature_page,
                    signature_pixel,
                    (SELECT COUNT(*) FROM soa_quotation_page WHERE quotation_id = soa_quotation.id and soa_quotation_page.status <> -1) page_count,
                    pageless,
                    po,
                    statement_date,
                    mode,
    ...
    $query = $query . " order by soa_quotation.created_at desc ";
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $mode = $row['mode'];
        $mode_content = $row['mode_content'];
        $caption = $row['caption'];
        $account_summary = $row['account_summary'];
        $caption_first_line = $row['caption_first_line'];
        $content_first_line = $row['content_first_line'];
...
function GetSubTotalInfo($qid, $db)
{
    ...
    $query = "
            select sum(amount) amt from soa_quotation_page_type_block
            WHERE type_id in (select id from soa_quotation_page_type where quotation_id = " . $qid . ")
            and status <> -1
    ";
...
function GetBlocks($qid, $db){
    $query = "
        ...
        num,
        pid
        FROM   soa_quotation_page_type_block
        WHERE  type_id = " . $qid . "
        AND `status` <> -1 
        ORDER BY id
    ";
