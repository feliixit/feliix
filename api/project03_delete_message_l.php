...
try{
    ...
    if($item_id != 0)
    {
        $query = "update project_other_task_message_reply_l
        SET
            status = -1,
    ...
    {
        $query = "update project_other_task_message_l
        SET
            status = -1,
            updated_id = :updated_id,
            updated_at = now()
...
function SendNotifyMail($last_id, $uid)
{
    ...
    message_notify_dept("del", $project_name, $task_name, $stages, $create_id, $assignee, $collaborator, "", $detail, $stage_id, $msg, $username, $created_at, $_id, 'LT');
...
function GetTaskDetail($id, $db)
{
    $sql = "SELECT pt.id, title task_name, 
            ...
            pmsg.created_at,
            pmsg.create_id _id
            FROM project_other_task_message_l pmsg
            LEFT JOIN project_other_task_l pt ON pmsg.task_id = pt.id
...
function GetReplyTaskDetail($id, $db)
{
    $sql = "SELECT pt.id, title task_name, 
            ...
            pmsgr.created_at,
            pmsgr.create_id _id
            FROM project_other_task_message_reply_l pmsgr
            LEFT JOIN project_other_task_message_l pmsg ON pmsg.id = pmsgr.message_id
            LEFT JOIN project_other_task_l pt ON pmsg.task_id = pt.id
            LEFT JOIN user u ON u.id = pmsg.create_id
            WHERE pmsgr.id = :id";
