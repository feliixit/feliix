...
$query = "SELECT pm.id, 
                ...
                (SELECT COUNT(*) FROM gcp_storage_file f where f.batch_id = pm.id AND f.batch_type = 'approval_form') AS q_count
...
if($app == "Y")
{
    $query = $query . " and (SELECT COUNT(*) FROM gcp_storage_file f where f.batch_id = pm.id AND f.batch_type = 'approval_form') > 0 ";
    $query_cnt = $query_cnt . " and (SELECT COUNT(*) FROM gcp_storage_file f where f.batch_id = pm.id AND f.batch_type = 'approval_form') > 0";
}
...
function GetRecentPost($quotation_id, $db){
    ...
    UNION all
              SELECT username, p.created_at updated_at  FROM approval_form_quotation_page p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM approval_form_quotation_page_type p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM approval_form_quotation_page_type_block p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.updated_at FROM approval_form_quotation_total p LEFT JOIN user u ON p.updated_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM approval_form_quotation_total p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM approval_form_quotation_term p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM approval_form_quotation_signature p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM approval_form_quotation_payment_term p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    ";
