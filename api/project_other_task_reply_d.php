...
switch ($method) {
    ...
    case 'POST':
        ...
        $query = "INSERT INTO project_other_task_message_reply_d
        SET
            `message_id` = :msg_id,
            `reply_id` = :reply_id,
            `message` = :message,
...
function SendNotifyMail($last_id, $uid)
{
    ...
    message_notify_dept("create", $project_name, $task_name, $stages, $create_id, $assignee, $collaborator, "", $detail, $stage_id, $msg, $username, $created_at, $_id, 'DS');
...
function GetTaskDetail($id, $db)
{
    $sql = "SELECT pt.id, title task_name, 
            ...
            potmr.created_at,
            potmr.create_id _id
            FROM project_other_task_message_reply_d potmr
            left join project_other_task_message_d pmsg on potmr.message_id = pmsg.id 
