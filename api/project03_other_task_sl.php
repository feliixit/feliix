...
switch ($method) {
    case "GET":
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $items = GetItemInfo($task_id, 'other_task_sl', $db);
...
function SendNotifyMail($last_id)
{
    ...
    task_notify_admin_sl("create", $project_name, $task_name, $stages, $create_id, $assignee, $collaborator, $due_date, $detail, $last_id, 0, 0, $created_at);
...
function GetMessage($task_id, $db, $uid)
{
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        // got it
        $got_it = GetGotIt($row['message_id'], 'sl', $db);
        foreach ($got_it as $g) {
            if ($g['uid'] == $uid) {
                $i_got_it = true;
                break;
...
function GetReply($msg_id, $db, $id, $name, $msg, $uid)
{
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        // got it
        $got_it = GetGotItReply($row['replay_id'], 'sl', $db);
