...
function GetQuotation($id, $db, $prefix) {
    ...
    $query = "SELECT id, 
                    ...
                    footer_first_line,
                    footer_second_line,
                    (SELECT COUNT(*) FROM transmittal_page WHERE quotation_id = transmittal.id and transmittal_page.status <> -1) page_count,
                    pageless
                    FROM transmittal
                    WHERE status <> -1 and id=$id";
...
function GetSubTotalInfo($qid, $db, $prefix)
{
    ...
    $query = "
            select COALESCE(sum(amount), 0) amt from transmittal_page_type_block
            WHERE `status` <> -1 and  type_id in (select id from transmittal_page_type where quotation_id = " . $qid . " and not_show = '' and real_amount = 0)
            union all
            select COALESCE(sum(real_amount), 0) from transmittal_page_type where quotation_id = " . $qid . " and not_show = '' and real_amount <> 0
    ";
...
function GetSubTotalInfoNotShowA($qid, $db, $prefix)
{
    ...
    $query = "
            select COALESCE(sum(amount), 0) amt from transmittal_page_type_block
            WHERE `status` <> -1 and type_id in (select id from transmittal_page_type where quotation_id = " . $qid . " and not_show = '' and block_type = 'A'  and real_amount = 0)
            union all
            select COALESCE(sum(real_amount), 0) from transmittal_page_type where quotation_id = " . $qid . " and not_show = '' and real_amount <> 0 and block_type = 'A' 
    ";
...
function GetSubTotalInfoNotShowB($qid, $db, $prefix)
{
    ...
    $query = "
            select COALESCE(sum(amount), 0) amt from transmittal_page_type_block
            WHERE `status` <> -1 and type_id in (select id from transmittal_page_type where quotation_id = " . $qid . " and not_show = '' and block_type = 'B'  and real_amount = 0)
            union all
            select COALESCE(sum(real_amount), 0) from transmittal_page_type where quotation_id = " . $qid . " and not_show = '' and real_amount <> 0 and block_type = 'B' 
    ";
...
function GetSubTotalNoVat($qid, $db, $prefix)
{
    ...
    $query = "
            select sum(qty * price * (1 - discount / 100) * ratio) amt from transmittal_page_type_block
            WHERE `status` <> -1 and type_id in (select id from transmittal_page_type where quotation_id = " . $qid . " and not_show = '' )
    ";
...
function GetSubTotalNoVatA($qid, $db, $prefix)
{
    ...
    $query = "
            select sum(qty * price * (1 - discount / 100) * ratio) amt from transmittal_page_type_block
            WHERE `status` <> -1 and type_id in (select id from transmittal_page_type where quotation_id = " . $qid . " and not_show = '' and block_type = 'A')
    ";
...
function GetSubTotalNoVatB($qid, $db, $prefix)
{
    ...
    $query = "
            select sum(qty * price * (1 - discount / 100) * ratio) amt from transmittal_page_type_block
            WHERE `status` <> -1 and type_id in (select id from transmittal_page_type where quotation_id = " . $qid . " and not_show = '' and block_type = 'B')
    ";
...
function GetSubTotalNoVatNotShow($qid, $db, $prefix)
{
    ...
    $query = "
            select COALESCE(sum(qty * ratio * price * (1 - discount / 100)), 0) amt from transmittal_page_type_block
            WHERE `status` <> -1 and type_id in (select id from transmittal_page_type where quotation_id = " . $qid . " and not_show = '')
    ";
...
function GetBlockNames($qid, $db, $prefix){
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $blocks = GetBlocks($qid, $db, $prefix);
...
$query .= "
        pid
        FROM  transmittal_page_type_block
        WHERE  quotation_id = " . $qid . "
        AND `status` <> -1 
        ORDER BY id
    ";
