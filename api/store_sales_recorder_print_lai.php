...
if($jwt){ 
    ...
    try {
        ...
        while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            $sheet->setCellValue('I1', 'Outside Sales');
            $sheet->setCellValue('J1', 'P.O. Number');
            $sheet->setCellValue('K1', 'DR#');
            ...
            foreach ($merged_results as $measure)
            {
                ...
                foreach($measure["payment"] as $rec)
                {
                    ...
                    $sheet->setCellValue('B' . $i, $measure["company"]);
                    $sheet->setCellValue('C' . $i, $measure["client"]);
                    $sheet->setCellValue('D' . $i, $rec["product_name"]);
                    $sheet->setCellValue('E' . $i, $rec["qty"]);
                    $sheet->setCellValue('F' . $i, $rec["price"]);
                    $sheet->setCellValue('G' . $i, $rec['qty'] * $rec['price']);
                    $sheet->setCellValue('H' . $i, $measure["total_amount"]);
                    $sheet->setCellValue('I' . $i, $measure["sales_name"]);
                    $sheet->setCellValue('J' . $i, $measure["po"]);
                    $sheet->setCellValue('K' . $i, $measure["dr"]);
                    $sheet->setCellValue('L' . $i, $measure["note"]);
