...
switch ($method) {
    ...
    case 'POST':
        ...
        // delete previous -1
        $query = "delete from transmittal_page_type_block 
                    WHERE
        ...
        // quotation_page
        $query = "UPDATE transmittal_page_type_block set `status` = -1
                    WHERE
                    `quotation_id` = :quotation_id";
        ...
        try {
            ...
            $query = "UPDATE transmittal_page_type p,( SELECT type_id, sum(amount)  as mysum FROM transmittal_page_type_block where `status` <> -1 GROUP BY type_id ) as s
            ...
            $query = "UPDATE transmittal SET `pageless` = :pageless, `updated_id` = :updated_id, `updated_at` = now() WHERE `id` = :id";
            $stmt = $db->prepare($query);
...
function UpdateImageNameVariation($upload_name, $batch_id, $db){
    ...
    $query = "update transmittal_page_type_block
