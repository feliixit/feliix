...
$query = "SELECT pm.id, 
            ...
            pm.task_type kind,
            (select stage_id from project_other_task where id = pm.task_id) stage_id,
            COALESCE((SELECT project_name FROM project_main WHERE id = (select project_id from project_stages where id = (select stage_id from project_other_task where id = pm.task_id) ) and pm.task_type = ''), '') AS project_name,
            COALESCE((SELECT title FROM project_other_task_a WHERE id = pm.task_id and pm.task_type = 'AD'), '') AS project_name_a,
            COALESCE((SELECT title FROM project_other_task_d WHERE id = pm.task_id and pm.task_type = 'DS'), '') AS project_name_d,
            COALESCE((SELECT title FROM project_other_task_l WHERE id = pm.task_id and pm.task_type = 'LT'), '') AS project_name_l,
            COALESCE((SELECT title FROM project_other_task_o WHERE id = pm.task_id and pm.task_type = 'OS'), '') AS project_name_o,
            COALESCE((SELECT title FROM project_other_task_sl WHERE id = pm.task_id and pm.task_type = 'SLS'), '') AS project_name_sl,
            COALESCE((SELECT title FROM project_other_task_sv WHERE id = pm.task_id and pm.task_type = 'SVC'), '') AS project_name_sv,
            pm.serial_name quotation_no,
...
if($kind != "")
{
    $query = $query . " and pm.task_type = '" . $kind . "' ";
}
...
function GetRecentPost($quotation_id, $db){
    $query = "SELECT username, p.updated_at FROM iq_main p LEFT JOIN user u ON p.updated_id = u.id  WHERE p.id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at  FROM iq_item p LEFT JOIN user u ON p.create_id = u.id  WHERE p.iq_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM iq_message p LEFT JOIN user u ON p.create_id = u.id  WHERE p.iq_id = " . $quotation_id . " and p.status <> -1  
    ";
