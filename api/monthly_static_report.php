    ...
    case 'GET':
        ...
        if($strDate != '' && $strEDate != "")
        {
            ...
            for($i = 0; $i < 1; $i++)
            {
                ...
                $merged_results[] =  GetManyMonth($strDate, $strEDate, $sale_person, $category, $db);
...
function GetManyMonth($strDate, $endDate, $sale_person, $category, $db)
{
    ...
    $report2 = GetMonthFailReport($PeriodStart, $PeriodEnd, $sale_person, $category, $db);
    $report3 = GetMonthProcessReport($PeriodStart, $PeriodEnd, $sale_person, $category, $db);
    $total1 = array(
    ...
    return array("date" => date("Y/m",strtotime($strDate . "first day of 0 month")), "report" => $report1, "fail" => $report2, "process" => $report3, "total" => $total1,);
}
...
function GetMonthFailReport($PeriodStart, $PeriodEnd, $sale_person, $category, $db){
    $sql = "SELECT pm.id pid, user.username,
                        pm.project_name,
                        pm.catagory_id,
                        CASE pm.catagory_id  
                    ...
                    AND pm.updated_at > '" . $PeriodStart . "' AND pm.updated_at < '" . $PeriodEnd . "'  ";
...
function GetMonthProcessReport($PeriodStart, $PeriodEnd, $sale_person, $category, $db){
    $sql = "SELECT pm.id pid, user.username,
                pm.project_name,
                pm.catagory_id,
                CASE pm.catagory_id  
            ...
            AND pm.id NOT in
            (SELECT id 
                        FROM (SELECT project_id id FROM project_proof WHERE received_date < '" . $PeriodEnd . "' AND STATUS = 1 AND received_date <> '') x
                    )  ";
