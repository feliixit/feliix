...
if($type == 'A')
    $query = "SELECT 0 is_checked, id, `leave` le, leave_type, start_date, start_time, end_date, end_time, CASE when STATUS = -3 then 'V'  when STATUS = -1 then 'W' when leave_type = 'D' then 'D' WHEN reject_id + re_reject_id > 0 THEN 'R' WHEN approval_id * re_approval_id > 0 THEN 'A'  WHEN approval_id * re_approval_id = 0 THEN 'P' END approval, reason, pic_url, created_at FROM apply_for_leave WHERE start_date >= '" . $sdate1 . "' and start_date < '" . $edate1 . "' and   uid = " . $user_id ;
else {
    # code...
