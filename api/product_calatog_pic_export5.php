  ...
  try {
      ...
      switch ($method) {
          case 'GET':
            ...
            // chunk merge_results into 5 elements with key: id-start_id-end
            $merged_results = array_chunk($merged_results, 5, false);


            for($i = 0; $i < count($merged_results); $i++)
            {
                $start = $merged_results[$i][0]['id'];
                $end = $merged_results[$i][count($merged_results[$i]) - 1]['id'];
                
                // push into $chunked_results
                $chunked_results[] = array("id" => $start . "_" . $end, "data" => $merged_results[$i]);

            }

            ob_clean();

            echo json_encode($chunked_results, JSON_UNESCAPED_SLASHES);
