...
$query = "SELECT pm.id, 
                ...
                (SELECT COUNT(*) FROM gcp_storage_file f where f.batch_id = pm.id AND f.batch_type = 'soa_block_image') AS q_count
...
if($app == "Y")
{
    $query = $query . " and (SELECT COUNT(*) FROM gcp_storage_file f where f.batch_id = pm.id AND f.batch_type = 'soa_block_image') > 0 ";
    $query_cnt = $query_cnt . " and (SELECT COUNT(*) FROM gcp_storage_file f where f.batch_id = pm.id AND f.batch_type = 'soa_block_image') > 0";
}
...
function GetRecentFiles($pid, $db){
    $sql = "SELECT f.id, pm.remark comment, COALESCE(f.filename, '') filename, COALESCE(f.bucketname, '') bucket, COALESCE(f.gcp_name, '') gcp_name, u.username, pm.created_at, pm.final_approve final_quotation FROM soa_project_approve pm left join user u on u.id = pm.create_id LEFT JOIN gcp_storage_file f ON f.batch_id = pm.id AND f.batch_type = 'soa_block_image' where pm.project_id = " . $pid . " and pm.status <> -1 and f.status <> -1";
    $stmt = $db->prepare( $sql );
...
function GetRecentPost($quotation_id, $db){
    $query = "SELECT username, p.updated_at FROM soa_quotation p LEFT JOIN user u ON p.updated_id = u.id  WHERE p.id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at  FROM soa_quotation_page p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM soa_quotation_page_type p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM soa_quotation_page_type_block p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.updated_at FROM soa_quotation_total p LEFT JOIN user u ON p.updated_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM soa_quotation_total p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM soa_quotation_term p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM soa_quotation_signature p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    UNION all
              SELECT username, p.created_at updated_at FROM soa_quotation_payment_term p LEFT JOIN user u ON p.create_id = u.id  WHERE p.quotation_id = " . $quotation_id . " and p.status <> -1  
    ";
